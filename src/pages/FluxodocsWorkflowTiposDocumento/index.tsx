...
import {
  fetchWorkflowTiposDocumento,
  inactivateWorkflowTiposDocumento,
} from "@/services/fluxodocsWorkflowTiposDocumento";
import type { FluxodocsWorkflowTipoDocumento } from "@/types/entities/Fluxodocs";
import {
  FluxodocsWorkflowTiposDocumentoModal,
  type FluxodocsWorkflowTiposDocumentoModalMode,
} from "./components/FluxodocsWorkflowTiposDocumentoModal";
...
export default function FluxodocsWorkflowTiposDocumentoPage() {
  ...
  const [data, setData] = useState<FluxodocsWorkflowTipoDocumento[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsWorkflowTiposDocumentoModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsWorkflowTipoDocumento | null>(null);

  function openModal(mode: FluxodocsWorkflowTiposDocumentoModalMode, r: FluxodocsWorkflowTipoDocumento | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchWorkflowTiposDocumento();
      setData(res.filter((r) => r.situacaoId === 1));
    } catch {
      toast.error("Erro ao carregar workflows por tipo de documento");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateWorkflowTiposDocumento(inactivatingId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Workflows por Tipo de Documento</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Workflows por Tipo de Documento
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de workflows por tipo de documento</CardTitle>
