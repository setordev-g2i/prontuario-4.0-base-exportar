...
import {
  fetchRegrasFluxo,
  inactivateRegrasFluxo,
} from "@/services/fluxodocsRegrasFluxo";
import type { FluxodocsRegraFluxo } from "@/types/entities/Fluxodocs";
import {
  FluxodocsRegrasFluxoModal,
  type FluxodocsRegrasFluxoModalMode,
} from "./components/FluxodocsRegrasFluxoModal";
...
export default function FluxodocsRegrasFluxoPage() {
  ...
  const [data, setData] = useState<FluxodocsRegraFluxo[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsRegrasFluxoModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsRegraFluxo | null>(null);

  function openModal(mode: FluxodocsRegrasFluxoModalMode, r: FluxodocsRegraFluxo | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchRegrasFluxo();
      setData(res.filter((r) => r.situacaoId === 1));
    } catch {
      toast.error("Erro ao carregar regras de fluxo");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateRegrasFluxo(inactivatingId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Regras de Fluxo</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Regras de Fluxo
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de regras de fluxo</CardTitle>
