...
export interface StatusItemFormValues {
  codigo: string;
  nome: string;
  ordem: number | null;
  cor: string;
...
interface Props {
  mode: "create" | "edit";
  readOnly?: boolean;
  initial?: FluxodocsStatusItem | null;
  onSubmit: (values: StatusItemFormValues) => void;
  onCancel: () => void;
  submitting?: boolean;
}

function buildInitialValues(entity: FluxodocsStatusItem | null | undefined): StatusItemFormValues {
  if (!entity) {
    return {
    codigo: "",
    nome: "",
...
export function StatusItemForm({
  ...
  const [values, setValues] = useState<StatusItemFormValues>(() => buildInitialValues(initial));
  ...
  function set<K extends keyof StatusItemFormValues>(key: K, value: StatusItemFormValues[K]) {
