...
import {
  fetchStatus,
  inactivateStatu,
} from "@/services/fluxodocsStatus";
import type { FluxodocsStatus } from "@/types/entities/Fluxodocs";
import {
  FluxodocsStatusModal,
  type FluxodocsStatusModalMode,
} from "./components/FluxodocsStatusModal";
...
export default function FluxodocsStatusPage() {
  ...
  const [data, setData] = useState<FluxodocsStatus[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsStatusModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsStatus | null>(null);

  function openModal(mode: FluxodocsStatusModalMode, r: FluxodocsStatus | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    ...
    try {
      ...
      toast.error("Erro ao carregar status do fluxo");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateStatu(inactivatingId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Status do Fluxo</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Status do Fluxo
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de status do fluxo</CardTitle>
