...
import {
  fetchAprovacoesJustificativa,
  inactivateAprovacoesJustificativa,
} from "@/services/fluxodocsAprovacoesJustificativa";
import type { FluxodocsAprovacaoJustificativa } from "@/types/entities/Fluxodocs";
import {
  FluxodocsAprovacoesJustificativaModal,
  type FluxodocsAprovacoesJustificativaModalMode,
} from "./components/FluxodocsAprovacoesJustificativaModal";
...
export default function FluxodocsAprovacoesJustificativaPage() {
  ...
  const [data, setData] = useState<FluxodocsAprovacaoJustificativa[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsAprovacoesJustificativaModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsAprovacaoJustificativa | null>(null);

  function openModal(mode: FluxodocsAprovacoesJustificativaModalMode, r: FluxodocsAprovacaoJustificativa | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchAprovacoesJustificativa();
      setData(res.filter((r) => r.situacaoId === 1));
    } catch {
      toast.error("Erro ao carregar aprovações de justificativa");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateAprovacoesJustificativa(inactivatingId);
  ...
  const visible = useMemo(() => {
    ...
    if (!term) return data;
    return data.filter((r) => normalize(String(r.justificativa ?? "")).includes(term));
  }, [data, debounced]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Aprovações de Justificativa</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Aprovações de Justificativa
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de aprovações de justificativa</CardTitle>
