...
import {
  fetchTiposItem,
  inactivateTiposItem,
} from "@/services/fluxodocsTiposItem";
import type { FluxodocsTipoItem } from "@/types/entities/Fluxodocs";
import {
  FluxodocsTiposItemModal,
  type FluxodocsTiposItemModalMode,
} from "./components/FluxodocsTiposItemModal";
...
export default function FluxodocsTiposItemPage() {
  ...
  const [data, setData] = useState<FluxodocsTipoItem[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsTiposItemModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsTipoItem | null>(null);

  function openModal(mode: FluxodocsTiposItemModalMode, r: FluxodocsTipoItem | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchTiposItem();
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateTiposItem(inactivatingId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Tipos de Item</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Tipos de Item
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de tipos de item</CardTitle>
