...
import {
  fetchParametrosIa,
  inactivateParametrosIa,
} from "@/services/fluxodocsParametrosIa";
import type { FluxodocsParametroIa } from "@/types/entities/Fluxodocs";
import {
  FluxodocsParametrosIaModal,
  type FluxodocsParametrosIaModalMode,
} from "./components/FluxodocsParametrosIaModal";
...
export default function FluxodocsParametrosIaPage() {
  ...
  const [data, setData] = useState<FluxodocsParametroIa[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsParametrosIaModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsParametroIa | null>(null);

  function openModal(mode: FluxodocsParametrosIaModalMode, r: FluxodocsParametroIa | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchParametrosIa();
      setData(res.filter((r) => r.situacaoId === 1));
    } catch {
      toast.error("Erro ao carregar parâmetros de ia");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateParametrosIa(inactivatingId);
  ...
  const visible = useMemo(() => {
    ...
    if (!term) return data;
    return data.filter((r) => normalize(String(r.nome ?? "")).includes(term) || normalize(String(r.chave ?? "")).includes(term));
  }, [data, debounced]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Parâmetros de IA</h1>
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Parâmetros de IA
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de parâmetros de ia</CardTitle>
