...
import {
  fetchParametrosSla,
  inactivateParametrosSla,
} from "@/services/fluxodocsParametrosSla";
import type { FluxodocsParametroSla } from "@/types/entities/Fluxodocs";
import {
  FluxodocsParametrosSlaModal,
  type FluxodocsParametrosSlaModalMode,
} from "./components/FluxodocsParametrosSlaModal";
...
export default function FluxodocsParametrosSlaPage() {
  ...
  const [data, setData] = useState<FluxodocsParametroSla[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsParametrosSlaModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsParametroSla | null>(null);

  function openModal(mode: FluxodocsParametrosSlaModalMode, r: FluxodocsParametroSla | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchParametrosSla();
      setData(res.filter((r) => r.situacaoId === 1));
    } catch {
      toast.error("Erro ao carregar parâmetros de sla");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateParametrosSla(inactivatingId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Parâmetros de SLA
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de parâmetros de sla</CardTitle>
