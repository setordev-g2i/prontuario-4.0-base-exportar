...
import {
  fetchPrioridades,
  inactivatePrioridade,
} from "@/services/fluxodocsPrioridades";
import type { FluxodocsPrioridade } from "@/types/entities/Fluxodocs";
import {
  FluxodocsPrioridadesModal,
  type FluxodocsPrioridadesModalMode,
} from "./components/FluxodocsPrioridadesModal";
...
export default function FluxodocsPrioridadesPage() {
  ...
  const [data, setData] = useState<FluxodocsPrioridade[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsPrioridadesModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsPrioridade | null>(null);

  function openModal(mode: FluxodocsPrioridadesModalMode, r: FluxodocsPrioridade | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchPrioridades();
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivatePrioridade(inactivatingId);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Prioridades
          </p>
        </div>
        ...
        <CardContent>
          <div className="rounded-md border">
            <Table>
              ...
              <TableCell>{r.peso != null && r.peso !== "" ? String(r.peso) : "—"}</TableCell>
              <TableCell>
                {r.cor ? (
