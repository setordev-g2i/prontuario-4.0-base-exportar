...
import {
  fetchTiposDocumento,
  inactivateTiposDocumento,
} from "@/services/fluxodocsTiposDocumento";
import type { FluxodocsTipoDocumento } from "@/types/entities/Fluxodocs";
import {
  FluxodocsTiposDocumentoModal,
  type FluxodocsTiposDocumentoModalMode,
} from "./components/FluxodocsTiposDocumentoModal";
...
export default function FluxodocsTiposDocumentoPage() {
  ...
  const [data, setData] = useState<FluxodocsTipoDocumento[]>([]);
  ...
  const [modalMode, setModalMode] = useState<FluxodocsTiposDocumentoModalMode>("create");
  const [selected, setSelected] = useState<FluxodocsTipoDocumento | null>(null);

  function openModal(mode: FluxodocsTiposDocumentoModalMode, r: FluxodocsTipoDocumento | null) {
    setModalMode(mode);
    setSelected(r);
    setModalOpen(true);
  }
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchTiposDocumento();
  ...
  async function confirmInactivate() {
    ...
    try {
      await inactivateTiposDocumento(inactivatingId);
  ...
  const visible = useMemo(() => {
    ...
    if (!term) return data;
    return data.filter((r) => normalize(String(r.nome ?? "")).includes(term) || normalize(String(r.categoria ?? "")).includes(term));
  }, [data, debounced]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configurações &gt; Fluxo de Documentos &gt; Tipos de Documento
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de tipos de documento</CardTitle>
